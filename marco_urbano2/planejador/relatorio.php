...
/* Apuração dos dados: compartilhada com relatorio-pdf.php para que a
   tela e o PDF nunca mostrem números diferentes. */
...
<div class="form-actions">
    <button class="btn-primary">📄 Ver na tela</button>
    <button class="btn-primary"
            formaction="relatorio-pdf.php?planejamento_id=<?= (int) $planejamentoSelecionado ?>">
        ⬇ Baixar PDF oficial
    </button>
</div>
</form>
...
<!-- O documento que vale é o PDF do servidor. O "Imprimir" do navegador
     saía diferente em cada máquina — margem, escala e versão do navegador
     mudavam onde caía a quebra de página, e foi assim que apareceu foto
     cortada ao meio. Por isso o botão de imprimir foi retirado daqui: o
     que está abaixo é pré-visualização, não a peça oficial. -->
<form method="post" action="relatorio-pdf.php?planejamento_id=<?= (int) $planejamentoSelecionado ?>"
      class="form-actions no-print" style="justify-content:flex-start;">
    <?php foreach ($dados as $d): ?>
    <input type="hidden" name="trechos[]" value="<?= (int) $d['t']['id'] ?>">
    <?php endforeach; ?>
    <button class="btn-primary">⬇ Baixar PDF oficial</button>
</form>

<div class="form-card no-print" style="border:1px solid #334155;">
    <strong style="color:#e2e8f0;">Abaixo é pré-visualização</strong>
    <p style="color:#94a3b8;font-size:13px;margin-top:6px;line-height:1.5;">
        Serve para conferir os números antes de emitir. O documento oficial é o
        <strong>PDF gerado pelo servidor</strong> — ele sai igual em qualquer
        máquina e nunca parte uma foto entre duas páginas.
    </p>
</div>
