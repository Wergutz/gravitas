...
if (isset($_SESSION['usuario_id']) && (int)($_SESSION['nivel'] ?? 0) === 5) {
    ...
    if (!hash_equals($_SESSION['csrf_token'], $tokenPost)) {
        ...
        if ($email && $senha) {
            $stmt = $pdo->prepare("
                ...
                FROM usuarios
                WHERE (email = ? OR nome = ?) AND ativo = 1 AND tipo_usuario = 5
                LIMIT 1
            ");
...
<title>Entrar · Executor · MARCO URBANO</title>
<style>
  :root{
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .ajuda{text-align:center;font-size:12.5px;color:var(--muted);margin-top:16px}
...
<div class="phone">
  ...
  <circle cx="50" cy="50" r="5" fill="#3CB86A"/>
    ...
    <b>MARCO URBANO</b><small>PAINEL DE CONTROLE</small>
  </div>

  <div class="eyebrow">&#9679; App do Executor</div>
  <p class="lead">Acesso do responsável de equipe.<br>A frente do dia e os lançamentos em um só lugar.</p>
    ...
    <form method="post" action="">
      ...
      <div class="field">
        ...
        <div class="inp">
          ...
          <input id="u" name="email" type="text" placeholder="equipe@gravitas.net.br ou seu nome" autocomplete="username" required>
        </div>
      </div>
